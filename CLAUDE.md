...
**Background sync** (optional, see "Remote sync protocol" below) — automatic, code-based sync between devices via the Cloudflare Worker. A privacy banner ("le code n'est pas un mot de passe…") is shown unconditionally whenever `VITE_SYNC_WORKER_URL` is set, in three places: the shared `Modal` shell (all 3 counter panels get it for free), `SyncPanel.tsx`'s own header, and the empty-counters state in `App.tsx`. It used to live only inside `SyncPanel`'s pre-code state, invisible to anyone already synced — keep it in all three spots if you touch any.
...
The 8-character code is a shared secret, not per-device authentication — anyone who has it can push arbitrary JSON, and the worker's `isValidPushRequest` only checks that `counters` is an array, never its elements' shape. Because of that, `remoteSync.ts` never trusts a network response as-is: `sync.ts`'s `sanitizeSyncedCounters` runs once, at the `fetchSyncState`/`pushSyncState` boundary (not scattered across `useRemoteSync.ts`'s several call sites), coercing every field to a safe default and validating `startDate` via `date.ts`'s `isValidIsoDate`. Before this existed, a malformed peer's push could crash the whole app — `Intl.DateTimeFormat.format` throws on an `Invalid Date`, and neither `CounterCard` nor `<main>` has its own `ErrorBoundary` to contain it. Route any new `SyncState`-reading code through the existing sanitizer rather than adding a call site that bypasses it. Similarly, `handlePut` measures the actual request body (`request.arrayBuffer().byteLength`) against `MAX_BODY_BYTES` rather than trusting the client-supplied `Content-Length` header, which can be omitted or understated.
